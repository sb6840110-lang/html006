...
<head>
    ...
    <title>ระบบเช็คชื่อกิจกรรมยุคดิจิทัล (Real-time Live Sync via Firebase)</title>
    <!-- Tailwind CSS CDN -->
    ...
    <!-- Firebase App and Realtime Database SDKs (Compat Mode) -->
...
<body class="bg-slate-100 font-sans min-h-screen pb-12">
    ...
    <header class="bg-indigo-700 text-white p-6 shadow-lg">
        <div class="max-w-5xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
            <div>
                ...
                <p class="text-indigo-200 text-sm">เข้าสู่ระบบเพื่อลงทะเบียน • ข้อมูลเชื่อมต่อ Firebase อัปเดตเรียลไทม์ทุกเครื่อง</p>
            </div>
        ...
        <section id="attendancePanel" class="hidden space-y-6">
            <div class="bg-indigo-50 border border-indigo-200 p-4 rounded-2xl flex items-center justify-between text-indigo-900">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h3 class="font-bold text-sm">ข้อมูลสรุปและประวัติการลงทะเบียนล่าสุด (Firebase Realtime)</h3>
                        <p class="text-xs text-indigo-700">เมื่อนักเรียนลงทะเบียนจากเครื่องใดก็ตาม ข้อมูลจะเด้งขึ้นหน้านี้ทันที</p>
                    </div>
                </div>
                <span class="text-xs bg-indigo-200 font-bold px-3 py-1 rounded-full border border-indigo-300">Live Sync</span>
            </div>
    ...
    <script>
        // ==========================================================
        // 1. CONFIG FIREBASE (นำค่าจาก Firebase Console มาวางตรงนี้)
        // ==========================================================
        ...
        try {
            ...
            console.error("Firebase Initialization Error: โปรดใส่ค่า Firebase Config ที่ถูกต้อง", e);
        }
        ...
        function listenToAttendanceUpdates() {
            if (!database) {
                console.warn("ไม่พบการเชื่อมต่อฐานข้อมูล Firebase");
                return;
            }

            // คอยดึงข้อมูลจาก Firebase โหนด 'attendances' เมื่อมีการเปลี่ยนแปลง
            database.ref('attendances').orderByChild('timestamp').on('value', (snapshot) => {
                attendanceData = [];
                snapshot.forEach((childSnapshot) => {
                    // ดึงข้อมูลมาใส่ด้านหน้า เพื่อให้รายการใหม่อยู่บนสุด
                    attendanceData.unshift(childSnapshot.val());
                });
        ...
        function loginUser(role, displayName) {
            ...
            // เริ่มรับฟังข้อมูลจาก Firebase Realtime Database
        ...
        // ==========================================================
        // 3. Form Submit Execution (Save to Firebase)
        // ==========================================================
        document.getElementById("checkinForm").addEventListener("submit", (e) => {
            ...
            const record = {
                ...
                timestamp: Date.now() // ใส่ Timestamp สำหรับจัดเรียงข้อมูล
            };
            ...
            // บันทึกไปยัง Firebase Realtime Database
            if (database) {
                database.ref('attendances').push(record)
                    .then(() => {
                    ...
                    .finally(() => {
                        ...
                        submitBtn.innerHTML = '<i class="fa-solid fa-paper-plane"></i> ยืนยันการบันทึกเวลา';
                    });
            } else {
                alert("ยังไม่ได้ตั้งค่า Firebase Config ให้ถูกต้อง");
                submitBtn.disabled = false;
                submitBtn.innerHTML = '<i class="fa-solid fa-paper-plane"></i> ยืนยันการบันทึกเวลา';
            }
        });
